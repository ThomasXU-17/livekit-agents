...
from livekit.plugins.openfacebot import STT, TTS, TTS_Volc
...
async def entrypoint(ctx: JobContext):
    ...
    # 移除可能存在的引号
    if volc_appid:
        volc_appid = volc_appid.strip("'\"")
    if volc_access_token:
        volc_access_token = volc_access_token.strip("'\"")
    
    if not volc_appid or not volc_access_token:
        raise ValueError("VOLC_APPID and VOLC_ACCESS_TOKEN not found in environment variables")
    
    logger.info(f"Volcengine APPID: {volc_appid[:4]}... (length: {len(volc_appid)})")
    logger.info(f"Volcengine Token: {volc_access_token[:4]}... (length: {len(volc_access_token)})")
    ...
    session = AgentSession(
        ...
        tts=TTS_Volc(
            ...
            voice="zh_female_shuangkuaisisi_moon_bigtts",  # 双快思思（女声）
            sample_rate=24000,
            encoding="pcm",
            speed=1.0,
            volume=1.0,
